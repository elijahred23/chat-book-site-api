...
const readSettings = () => {
  try {
    ...
    return { tab: "tutor", dataset: "vocab", mode: "bengali", intervalSize: 10, intervalRepeats: 2, loopForever: true };
  }
};

export default function BengaliTutorPersistent() {
  const initial = useMemo(readSettings, []);
  const [tab, setTab] = useState(initial.tab);
  const [voices, setVoices] = useState([]);
  const [bnVoice, setBnVoice] = useState(() => readString(BN_VOICE_KEY));
  const [enVoice, setEnVoice] = useState(() => readString(EN_VOICE_KEY));
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(SETTINGS_KEY, JSON.stringify({ ...current, tab }));
    } catch {}
  }, [tab]);
  ...
  return (
    <main style={ui.page}>
      <nav style={ui.tabs} aria-label="Bengali tutor sections">
        <button type="button" style={tab === "tutor" ? ui.activeTab : ui.tab} onClick={() => setTab("tutor")}>Tutor</button>
        <button type="button" style={tab === "loop" ? ui.activeTab : ui.tab} onClick={() => setTab("loop")}>Word Loop</button>
      </nav>
      {tab === "tutor" ? (
        <>
...
function WordLoop({ initial, voices, bnVoices, enVoices, bnVoice, enVoice, setBnVoice, setEnVoice, preview }) {
  const [lesson] = useState(readLesson);
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(SETTINGS_KEY, JSON.stringify({
        ...
        dataset,
        mode,
        intervalSize: Math.max(1, Number(intervalSize) || 1),
        intervalRepeats: Math.max(1, Number(intervalRepeats) || 1),
        loopForever,
      }));
    } catch {}
  }, [dataset, mode, intervalSize, intervalRepeats, loopForever]);
  ...
  return (
    <section style={ui.card}>
      <div>
        <strong style={ui.eyebrow}>Hands-free practice</strong>
        <h1 style={ui.heading}>Bengali Word Loop</h1>
        <p style={ui.muted}>Repeat each interval before moving to the next set. English stays visible in every reading mode.</p>
      </div>
      <div style={ui.grid}>
      ...
      {!lesson ? <div style={ui.notice}>Generate or upload a lesson in the Tutor tab first.</div> : !items.length ? <div style={ui.notice}>No items are available in this lesson.</div> : <>
        ...
        <div style={ui.actions}>
          <button type="button" style={ui.primary} onClick={start} disabled={playing && !paused}>{paused ? "Resume" : playing ? "Playing" : "Start"}</button>
          <button type="button" style={ui.secondary} onClick={togglePause} disabled={!playing}>{paused ? "Resume" : "Pause"}</button>
          <button type="button" style={ui.secondary} onClick={() => stop()} disabled={!playing && !paused}>Stop</button>
          <button type="button" style={ui.secondary} onClick={restart}>Restart</button>
        </div>
...
const ui = {
  ...
  tabs: { width: "min(100% - 2rem, 1100px)", margin: "1rem auto 0", padding: ".35rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".35rem", background: "#e8eef8", border: "1px solid #cbd5e1", borderRadius: 14 },
  tab: { minHeight: 44, border: "1px solid transparent", borderRadius: 10, background: "transparent", color: "#334155", fontWeight: 800, cursor: "pointer", WebkitTextFillColor: "#334155" },
  activeTab: { minHeight: 44, border: "1px solid #94a3b8", borderRadius: 10, background: "#ffffff", color: "#0f172a", fontWeight: 900, cursor: "pointer", boxShadow: "0 3px 10px rgba(15,23,42,.1)", WebkitTextFillColor: "#0f172a" },
  voice: { width: "min(100% - 2rem, 1100px)", margin: ".75rem auto 0", padding: "1rem", border: "1px solid #dbe3ef", borderRadius: 14, background: "#fff", color: "#0f172a" },
  voiceCompact: { padding: ".85rem", border: "1px solid #dbe3ef", borderRadius: 12, background: "#f8fafc", color: "#0f172a" },
  card: { width: "min(100% - 2rem, 1100px)", margin: ".75rem auto 1.5rem", padding: "1rem", display: "grid", gap: "1rem", border: "1px solid #dbe3ef", borderRadius: 16, background: "#fff", color: "#0f172a" },
  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: ".75rem" },
  field: { display: "grid", gap: ".4rem", color: "#0f172a" },
  input: { width: "100%", minHeight: 44, padding: ".65rem .75rem", border: "1px solid #94a3b8", borderRadius: 10, background: "#fff", color: "#0f172a", WebkitTextFillColor: "#0f172a", fontWeight: 700 },
  check: { display: "flex", gap: ".55rem", alignItems: "center", color: "#0f172a", fontWeight: 800 },
  flash: { minHeight: 180, padding: "1.25rem", display: "grid", placeItems: "center", alignContent: "center", gap: ".45rem", border: "1px solid #93c5fd", borderRadius: 16, background: "linear-gradient(145deg,#eff6ff,#f0fdf4)", color: "#0f172a", textAlign: "center" },
  ...
  primary: { minHeight: 44, padding: ".65rem 1rem", border: "1px solid #0f172a", borderRadius: 10, background: "#0f172a", color: "#fff", WebkitTextFillColor: "#fff", fontWeight: 850, cursor: "pointer" },
  secondary: { minHeight: 44, padding: ".65rem 1rem", border: "1px solid #94a3b8", borderRadius: 10, background: "#f8fafc", color: "#0f172a", WebkitTextFillColor: "#0f172a", fontWeight: 800, cursor: "pointer" },
