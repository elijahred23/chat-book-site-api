...
export default function BengaliTutorAdvanced() {
  const [tab, setTab] = useState("tutor");
  const [voices, setVoices] = useState([]);
  const [bnVoice, setBnVoice] = useState(() => saved(BN_VOICE_KEY));
  const [enVoice, setEnVoice] = useState(() => saved(EN_VOICE_KEY));
  ...
  return (
    <main style={ui.page}>
      <nav style={ui.tabs}>
        <button style={tab === "tutor" ? ui.active : ui.tab} onClick={() => setTab("tutor")}>Tutor</button>
        <button style={tab === "loop" ? ui.active : ui.tab} onClick={() => setTab("loop")}>Word Loop</button>
      </nav>
      {tab === "tutor" ? (
        <>
          <VoiceSelect label="Bengali voice" value={bnVoice} voices={bnVoices} onChange={(value) => {
            setBnVoice(value);
            if (value) preview(value, "স্বাগতম", "bn-IN");
          }} />
          <BengaliTutor />
        </>
      ) : (
...
function WordLoop({ voices, bnVoices, enVoices, bnVoice, enVoice, setBnVoice, setEnVoice, preview }) {
  ...
  const [mode, setMode] = useState("bengali");
  const [intervalSize, setIntervalSize] = useState(10);
  const [intervalRepeats, setIntervalRepeats] = useState(2);
  ...
  const readItem = useCallback(async (item) => {
    ...
    if (mode === "bengali-english" && playingRef.current && !pausedRef.current) {
      await speak(item.en, "en-US", enVoice);
    }
  ...
  return (
    <section style={ui.card}>
      <div><strong style={ui.eyebrow}>Hands-free practice</strong><h1 style={{ margin: ".25rem 0" }}>Bengali Word Loop</h1>
        <p style={ui.muted}>Repeat each interval before moving to the next set. English stays visible in every reading mode.</p></div>
      <div style={ui.grid}>
        <Field label="Items"><select style={ui.input} value={dataset} onChange={(event) => setDataset(event.target.value)}>
          <option value="vocab">Vocabulary words</option><option value="phrases">Lesson phrases</option></select></Field>
        <Field label="Reading mode"><select style={ui.input} value={mode} onChange={(event) => setMode(event.target.value)}>
          <option value="bengali">Bengali only</option><option value="bengali-english">Bengali, then English</option>
          <option value="english-bengali">English, then Bengali</option></select></Field>
      ...
      {!lesson ? <div style={ui.notice}>Generate or upload a lesson in the Tutor tab first.</div> : !items.length ? <div style={ui.notice}>No items are available in this lesson.</div> : <>
        <div style={ui.flash}><small>{currentIndex + 1}/{items.length} · Set {Math.floor(currentIndex / size) + 1}/{Math.ceil(items.length / size)} · Pass {pass}/{repeats}</small>
          <strong lang="bn" style={ui.bn}>{item?.bn}</strong><span style={ui.en}>{item?.en}</span>{item?.pronunciation && <span style={ui.muted}>{item.pronunciation}</span>}</div>
        <div style={ui.actions}><button style={ui.primary} onClick={start} disabled={playing && !paused}>{paused ? "Resume" : playing ? "Playing" : "Start"}</button>
          <button style={ui.button} onClick={togglePause} disabled={!playing}>{paused ? "Resume" : "Pause"}</button>
          <button style={ui.button} onClick={() => stop()} disabled={!playing && !paused}>Stop</button><button style={ui.button} onClick={restart}>Restart</button></div>
...
function VoiceSelect({ label, value, voices, onChange, compact }) {
  return <section style={compact ? ui.voiceCompact : ui.voice}><Field label={label}><select style={ui.input} value={value} onChange={(event) => onChange(event.target.value)}>
    <option value="">System default</option>{voices.map((voice) => <option key={voiceKey(voice)} value={voiceKey(voice)}>{voice.name} ({voice.lang})</option>)}</select></Field></section>;
}
function Field({ label, children }) { return <label style={ui.field}><strong>{label}</strong>{children}</label>; }

const ui = {
  page: { minHeight: "100%" }, tabs: { width: "min(100% - 2rem, 1100px)", margin: "1rem auto 0", padding: ".35rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".35rem", background: "#eef2f7", borderRadius: 14 },
  tab: { minHeight: 44, border: 0, borderRadius: 10, background: "transparent", fontWeight: 800 }, active: { minHeight: 44, border: "1px solid #dbe3ef", borderRadius: 10, background: "#fff", fontWeight: 900 },
  ...
  card: { width: "min(100% - 2rem, 1100px)", margin: ".75rem auto 1.5rem", padding: "1rem", display: "grid", gap: "1rem", border: "1px solid #dbe3ef", borderRadius: 16, background: "#fff" }, grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: ".75rem" },
  field: { display: "grid", gap: ".4rem" }, input: { width: "100%", minHeight: 44, padding: ".65rem .75rem", border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", fontWeight: 700 }, check: { display: "flex", gap: ".55rem", alignItems: "center", fontWeight: 800 },
  flash: { minHeight: 180, padding: "1.25rem", display: "grid", placeItems: "center", alignContent: "center", gap: ".45rem", border: "1px solid #bfdbfe", borderRadius: 16, background: "linear-gradient(145deg,#eff6ff,#f0fdf4)", textAlign: "center" }, bn: { fontSize: "clamp(2rem,7vw,4rem)", lineHeight: 1.2 }, en: { color: "#1e3a8a", fontSize: "1.2rem", fontWeight: 850 },
  actions: { display: "flex", flexWrap: "wrap", gap: ".6rem" }, primary: { minHeight: 44, padding: ".65rem 1rem", border: 0, borderRadius: 10, background: "#0f172a", color: "#fff", fontWeight: 850 }, button: { minHeight: 44, padding: ".65rem 1rem", border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", fontWeight: 800 },
