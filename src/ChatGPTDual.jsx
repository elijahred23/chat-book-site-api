...
// Use a unique prefix so Chat 2 data never collides with the main chat's localStorage keys
...
export default function ChatGPTDual({ isCollapsed = false, hidePrompt = false, isOpen = false, onClose = () => {}, onToggleCollapse = () => {}, onTogglePrompt = () => {} }) {
  const [messages, setMessages] = useLocalStorageState(`${STORAGE_PREFIX}messages`, []);
  ...
  const [promptText, setPromptText] = useLocalStorageState(`${STORAGE_PREFIX}prompt`, "");
  const setPrompt = (value) => {
    setPromptText(value);
    dispatch(actions.setChat2Prompt(value));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      dispatch(actions.setChat2Prompt(""));
      setActiveTab("chat");
    } catch {
  ...
  // Keep prompt input in sync with context (e.g., when sent from ActionButtons)
  useEffect(() => {
    if (chat2Prompt !== undefined && chat2Prompt !== promptText) {
      setPromptText(chat2Prompt);
    }
  }, [chat2Prompt, promptText]);
  ...
  const styles = `
    ...
    .gpt-shell-dual .floating-controls {
      position: fixed;
      top: 10px;
      right: 10px;
      z-index: 100001;
    ...
    .gpt-shell-dual .btn-primary {
      background: linear-gradient(135deg, #16a34a, #22c55e);
      color: #0b1220;
      border: none;
      box-shadow: 0 10px 24px rgba(34,197,94,0.25);
    }
    .gpt-shell-dual .btn-ghost {
      background: #e3f9ed;
      border-color: #bbf7d0;
      color: #065f46;
    }
    .gpt-shell-dual .select-btn {
      padding: 0.6rem 0.85rem;
      border-radius: 12px;
      border: 1px solid #bbf7d0;
      background: #ecfdf3;
    ...
    .gpt-shell-dual .suggestion.selected {
      border-color: #16a34a;
      background: #dcfce7;
  ...
  return (
    <div>
      <style>{styles}</style>
      <div className="gpt-shell gpt-shell-dual">
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem" }}>
          ...
          <button className="btn btn-ghost" onClick={clearMessages}>Clear</button>
        </div>
          ...
          {!hidePrompt && activeTab === "prompt" && (
            <div className="gpt-card">
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center", marginBottom: "0.5rem" }}>
                ...
                <button className="btn btn-ghost" onClick={() => setPrompt("")}>Clear prompt</button>
              </div>
              <div
                style={{
                  display: "flex",
