...
const instructionRows = [
  ["0000", "NOP", "No operation"], ["0001", "LDI A,n", "Load n into A"],
  ["0010", "LDI B,n", "Load n into B"], ["0011", "LDA n", "A ← RAM[n]"],
  ["0100", "STA n", "RAM[n] ← A"], ["0101", "ADD", "A ← A + B"],
  ["0110", "SUB", "A ← A − B"], ["0111", "AND", "A ← A AND B"],
  ["1000", "OR", "A ← A OR B"], ["1001", "XOR", "A ← A XOR B"],
  ["1010", "JMP n", "Jump to n"], ["1011", "JZ n", "Jump if zero"],
  ["1100", "JC n", "Jump if carry"], ["1101", "OUT", "Output ← A"],
  ["1110", "SWP", "Swap A and B"], ["1111", "HLT", "Stop clock"],
];

const languageHelp = {
  binary: "Enter one or more 8-bit bytes per line.",
...
function CpuExecutionModal({ state, running, busy, onClose, onStep, onToggleRunning }) {
  ...
  const controlActive = signals.includes("CONTROL DECODE") || signals.includes("ZERO TEST") || signals.includes("CARRY TEST");
        ...
        <div className="cpu-circuit-scroll" aria-label={`CPU circuit after ${state.lastEvent.phase.toLowerCase()} phase`}>
          <svg className="cpu-circuit-board" viewBox="0 0 1000 610" role="img" aria-labelledby="cpu-circuit-title cpu-circuit-description">
            ...
            <g className="cpu-circuit-wires">
              ...
              <path className={signals.includes("ZERO TEST") || signals.includes("CARRY TEST") ? "active control" : "control"} d="M780 435 H820 V315 H640" />
            ...
            <CircuitNode x={455} y={35} width={180} height={155} label="RAM · 16 BYTES" value={`[${state.memoryAddressRegister.toString(16).toUpperCase()}] ${bits(state.memory[state.memoryAddressRegister])}`} detail="SHARED PROGRAM / DATA" active={memoryActive} accent="lime" />
            ...
            <CircuitNode x={250} y={440} label="REGISTER B" value={`${bits(state.registerB)} · ${state.registerB}`} detail="GENERAL REGISTER" active={hasPrefix("B ")} />
            ...
            <CircuitNode x={700} y={390} width={160} label="FLAGS" value={`Z ${Number(state.zeroFlag)} · C ${Number(state.carryFlag)}`} detail="ZERO / CARRY" active={signals.includes("FLAGS IN") || signals.includes("ZERO TEST") || signals.includes("CARRY TEST")} accent="orange" />
            <text className="cpu-wire-label" x="205" y="96">ADDRESS</text><text className="cpu-wire-label" x="342" y="342">8-BIT DATA BUS</text><text className="cpu-wire-label" x="662" y="542">OUTPUT BUS</text>
          </svg>
        </div>
  ...
  return (
    <div className="cpu-sim">
      <header className="cpu-topbar">
        <div className="cpu-brand"><span className="cpu-brand__mark">01</span><div><b>Clockwork</b><small>8-bit CPU laboratory</small></div></div>
      ...
      <main className="cpu-main">
        ...
        <div className="cpu-workbench">
          <aside className="cpu-program-panel cpu-panel">
            <div className="cpu-panel-title"><span>01</span><div><h2>Program</h2><p>16 bytes maximum</p></div></div>
            ...
            {language === "simple" && <details className="cpu-language-guide"><summary>MiniScript syntax</summary><code>let count = 3;</code><code>const slot = 15;</code><code>count += step;</code><code>while (count !== 0) {"{ … }"}</code><code>output(count);</code></details>}
          ...
          <aside className="cpu-memory-panel cpu-panel">
            <div className="cpu-panel-title"><span>02</span><div><h2>Memory</h2><p>16 × 8-bit RAM</p></div></div>
            {state ? <MemoryGrid state={state} /> : <div className="cpu-memory-grid disabled">{Array.from({ length: 16 }, (_, address) => <div className="cpu-memory-cell" key={address}><div className="cpu-memory-address"><span>{address.toString(16).toUpperCase()}</span></div><b>00000000</b><small>0x00</small></div>)}</div>}
        ...
        <section className="cpu-reference cpu-panel">
          ...
          <div className="cpu-instructions"><h3>Instruction set</h3><p>High nibble = opcode · low nibble = operand n</p><div className="cpu-instruction-grid">{instructionRows.map(([opcode, name, description]) => <div key={opcode}><code>{opcode}</code><b>{name}</b><span>{description}</span></div>)}</div></div>
        </section>
      </main>
      ...
      <footer className="cpu-footer"><span>Clockwork CPU Lab</span><p>An educational 8-bit architecture. Values wrap at 255; addresses wrap at 15.</p></footer>
    </div>
  );
}
